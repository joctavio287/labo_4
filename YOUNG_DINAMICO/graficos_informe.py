...
class regresion_lineal:
    ...
    # def fit(self, x, y, error_y = None):
    #     self.x, self.y = x, y
    #     N = len(x)
    #     DELTA = N*np.sum(x**2) - np.sum(x)**2
    #     ordenada = (np.sum(x**2)*np.sum(y) - np.sum(x)*np.sum(x*y))/DELTA
    #     pendiente = (N*np.sum(x*y) - np.sum(x)*np.sum(y))/DELTA
    #     matriz_de_cov = np.array([[np.sum(x**2), -np.sum(x)],[-np.sum(x), N]])
    #     matriz_de_cov *= (error_y**2/DELTA)
    #     self.parametros, self.cov_parametros = np.array([ordenada, pendiente]), matriz_de_cov
    
    def bondad(self):
        self.r = np.corrcoef(self.x, self.y)

#######################################################
df = pd.read_csv('C:/repos/labo_4/YOUNG_DINAMICO/Mediciones/medicio_osci{}.csv'.format(str(1)))
...
error = 0.00625*1.5 # 1.6V/256 + 1.6V/256 * .5 # AGREGO UN %50 de error
...
with plt.style.context('seaborn-whitegrid'):
    ...
    ax.scatter(picos_t, np.log(amplitud_t), marker = '.', color = 'k', label = 'Datos')
    ax.errorbar(picos_t, np.log(amplitud_t), marker = '.', yerr = error, fmt = 'none', capsize = 2, color = 'black', label = 'Error de los datos')
...
# RUIDO:
df = pd.read_csv('C:/repos/labo_4/YOUNG_DINAMICO/Mediciones/medicio_daq0.csv')

with plt.style.context('seaborn-whitegrid'):
    fig, axs = plt.subplots(nrows = 2, ncols = 2, figsize = (8, 7), sharey = False)
    axs = axs.flatten()
    datos = [d for d in df.tension]
    tiempo = np.array([t for t in df.tiempo])

    # Señal DAQ:
    axs[0].plot(tiempo, datos, label = 'Señal DAQ')
    axs[0].set_xlabel('Tiempo [s]', fontsize = 13)
    axs[0].set_ylabel('Tensión [V]', fontsize = 13)
    axs[0].set_xlim(0, 1)      
    axs[0].legend(fontsize = 11, loc = 'best')

    # Transformada entera señal DAQ:
    tstep = (tiempo.max()-tiempo.min())/len(tiempo)
    fsamp = 1/tstep # frecuencia de sampleo [HZ]
    señal_fft, N = np.fft.fft(datos), len(datos)
    xf = np.linspace(0, fsamp/2, int(N/2))
    yf = (2*np.abs(señal_fft[:N//2])/N)
    axs[2].plot(xf, yf, label = 'Transformada señal DAQ')
    ...
    picos_x, var_inservible = find_peaks(yf, prominence = (.5e-3, 1))
    i = 0
    for x_p, y_p in zip([xf[x] for x in picos_x], [yf[x] for x in picos_x]):
        if i<=3 and int(x_p) > 49:
            i+=1
            axs[2].plot(x_p, y_p, marker = "o", markersize = 5,
            label = 'Coordenadas: ({}, {})'.format(np.round(x_p, 2), np.round(y_p, 4)))
    axs[2].legend(fontsize = 11, loc = 'best')
    
    # Señal ruido DAQ:
    df = pd.read_csv('C:/repos/labo_4/YOUNG_DINAMICO/Mediciones/medicio_daqruido.csv')
    # Esto es para encontrar cuándo arranca y termina la señal limpia
    datos = [d for d in df.tension]
    tiempo = np.array([t for t in df.tiempo])
    axs[1].plot(tiempo, datos, label = 'Ruido DAQ')
    axs[1].set_xlabel('Tiempo [s]', fontsize = 13)
    # axs[1].set_ylabel('Tensión [V]', fontsize = 13)
    axs[1].set_xlim(0, 1)      
    axs[1].legend(fontsize = 11, loc = (.4,.9))

    # Transformada entera ruido DAQ:
    tstep = (tiempo.max()-tiempo.min())/len(tiempo)
    fsamp = 1/tstep # frecuencia de sampleo [HZ]
    señal_fft, N = np.fft.fft(datos), len(datos)
    xf = np.linspace(0, fsamp/2, int(N/2))
    yf = (2*np.abs(señal_fft[:N//2])/N)
    axs[3].plot(xf, yf, label = 'Transformada ruido')
    ...
    i = 0
    for x_p, y_p in zip([xf[x] for x in picos_x], [yf[x] for x in picos_x]):
        if i<=3 and int(x_p) > 15:
            i+=1
            axs[3].plot(x_p, y_p, marker = "o", markersize = 5,
            label = 'Frecuencia: {}'.format(np.round(x_p, 2)))
    axs[3].legend(fontsize = 11, loc = 'best')
...
# fig.show()
# fig.savefig('C:/Users/jocta/Documents/LaTex/Modulo_young/ruido.jpg', dpi=1200)